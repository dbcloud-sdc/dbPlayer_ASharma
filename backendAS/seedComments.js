...
class ReadComments extends Readable {
    ...
    if (this.songsGoal <= this.songRowsGenerated) {
      ...
      for (let j = 0; j < 35; j++) {
        ...
        for (let i = 0; i < randomCommentNumber; i++) {
          ...
          container += `${this.commentsGenerated},${songid},${commentimage},${comment},${commenttime},${username}\n`;
        }
  ...
  async song(length) {
    ...
    songRow += `${this.songRowsGenerated},${songname},${artistname},${imgurl},${hashtag},${timeelapsed},${length},${decibel},${songurl}\n`;
